# [LV_3] [재풀이 필요] 업그레이드 할 수 없는 아이템 구하기
...
### ✨ 핵심 포인트
- A를 B로 업그레이드하면 A는 부모가 된다. 
- 따라서 `ITEM_ID = PARENT_ITEM_ID`로 매칭되는 아이템은 ‘업그레이드가 가능한 아이템(자식이 있는 중간/상위 단계)’이다.
- 이들을 제외하면 ‘더 이상 업그레이드할 수 없는 **말단(leaf) 아이템**만 남는다’.
...
### 💡 풀이 과정
- `LEFT JOIN`을 사용하여 `ITEM_INFO`의 `ITEM_ID`와 `ITEM_TREE`의 `PARENT_ITEM_ID`를 매칭.
- 매칭되는 `PARENT_ITEM_ID`가 `NULL`인 경우, 해당 `ITEM_ID`는 어떤 아이템의 부모도 아니라는 뜻이므로 '말단 아이템'임.
